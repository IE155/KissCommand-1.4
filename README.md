...
KissCommand是一个Paper 1.21.x的插件，允许玩家 `/kiss <玩家名>` 指令向其他玩家发送一个可见消息和特效。
...
- 光环渲染默认关闭,需要在config.yml里开启

---

## 安装方法

1. 将 `KissCommand-1.3-SNAPSHOT.jar` 文件放入服务器的 `plugins/` 文件夹。
...
| `/kiss2load`|在服务器输入，无需重启，直接重新加载config.yml,只有op或者控制台才能执行/可见|`/kiss2load`|
